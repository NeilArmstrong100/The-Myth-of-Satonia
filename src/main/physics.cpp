...
constexpr float DAY_SPD = 0.01f; // Speed of day/night cycle
constexpr float DAY = 0.0f, NIGHT = 0.8f;
...
void run()
{
    ...
    while (thread::running)
    {
        ...
        if (key_manager::paused || !registry || !map::map_loaded)
        {
...
void physics_update(const float dt)
{
    ...
    for (auto& pair : transforms)
    {
        ...
        horizontal_colliders(tiles, transforms, collision, pos, velocity, size, entity);
		vertical_colliders(tiles, transforms, collision, pos, velocity, size, entity);
...
void event_handler(const float dt)
{
    ...
    if (sun_rise && day < NIGHT)
        day += DAY_SPD * dt;
    else if (sun_rise && day >= NIGHT)
        sun_rise = false;
    else if (!sun_rise && day > DAY)
		day -= DAY_SPD * dt;
	else if (!sun_rise && day <= DAY)
        sun_rise = true;
...
void horizontal_colliders(auto& tiles, auto& transforms, Collision* collision, Point& pos, Point& velocity, Point& size, Entity entity)
{
    pos.x += velocity.x;
...
void vertical_colliders(auto& tiles, auto& transforms, Collision* collision, Point& pos, Point& velocity, Point& size, Entity entity)
{
    pos.y += velocity.y;
...
void teleport_event()
{
    ...
    for (auto& pair : registry->get_all<Teleport>())
    {
            ...
            py + (ph / 2) <= ty + th && py + (ph / 2) >= ty)
        {
            ...
            map::map_to_load = teleport->map_to_load;
            map::map_loaded = false;
        }
    }
}