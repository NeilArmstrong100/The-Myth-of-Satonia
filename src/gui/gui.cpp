...
inline constinit stbtt_bakedchar cdata[96]{}; // ASCII 32..126
inline constinit GLuint ftex = 0;

inline constinit float util::settings::brightness = 0.925f;
inline constinit bool util::settings::fullscreen = false;
inline constexpr float LETTER_WIDTH = 6.0f;
...
void box_render(Box& box, bool& selecting)
{
    ...
    if (util::mouse_manager::x >= box.get_position().x &&
        util::mouse_manager::x <= box.get_position().x + box.get_bounds().x &&
        util::mouse_manager::y >= box.get_position().y &&
        util::mouse_manager::y <= box.get_position().y + box.get_bounds().y)
    {
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);

        if (util::mouse_manager::left && !selecting)
...
void button_render(Button& button, Point offset, float scale, float r = 0.6f, float g = 0.8f, float b = 1.0f, float a = 1.0f)
{
    ...
    if (util::mouse_manager::x >= button.get_position().x &&
        util::mouse_manager::x <= button.get_position().x + button.get_bounds().x &&
        util::mouse_manager::y >= button.get_position().y &&
        util::mouse_manager::y <= button.get_position().y + button.get_bounds().y)
    {
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        if (util::mouse_manager::left)
            button.get_callback()();
...
void slider_render(Slider& slider, bool& adjusting)
{
    ...
    if (util::mouse_manager::x >= slider.get_position().x &&
        util::mouse_manager::x <= slider.get_position().x + slider.get_bounds().x &&
        util::mouse_manager::y >= slider.get_position().y &&
        util::mouse_manager::y <= slider.get_position().y + slider.get_bounds().y)
    {
        if (util::mouse_manager::left)
            adjusting = true;
    }

    if (util::mouse_manager::left == false)
        adjusting = false;

    if (adjusting)
        slider.set_slider_position((float)(util::mouse_manager::x - (slider.get_slider_bounds().x / 2.0f)), slider.get_slider_position().y);
...
void settings_render(std::shared_ptr<Registry> registry, float width, float height, bool& visible)
{
    ...
    util::settings::brightness = (max - min) * ((sx - x) / w) + min;
    glfwSwapInterval(util::settings::v_sync);
    v_sync.is_toggled() ? util::settings::v_sync = 1 : util::settings::v_sync = 0;
	fullscreen.is_toggled() ? util::settings::fullscreen = true : util::settings::fullscreen = false;
