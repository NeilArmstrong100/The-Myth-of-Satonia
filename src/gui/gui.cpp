...
float util::settings::brightness = 0.0f;
...
void settings_render(Registry& registry, float width, float height, bool& visible)
{
    ...
    Button brightness_high
    (
        Point{ width / 2 + 160, height / 2 + 15 },
        Point{ 75, 50 },
        std::string("HIGH"),
        nullptr
    );

    Button brightness_medium
    (
    ...
    Button brightness_low
    (
        Point{ width / 2 - 230, height / 2 + 15 },
        Point{ 75, 50 },
        std::string("LOW"),
        nullptr
    );
    ...
    draw_text
    (
        exit.get_position().x / 1.5,
        exit.get_position().y / 1.5 + 20,
        exit.get_text().c_str(),
        1.5f
    );

    glColor4f(0.6f, 0.8f, 1.0f, 1.0f);
    // Brightness high button

    if (util::mouse_manager::x >= brightness_high.get_position().x &&
        util::mouse_manager::x <= brightness_high.get_position().x + brightness_high.get_bounds().x &&
        util::mouse_manager::y >= brightness_high.get_position().y &&
        util::mouse_manager::y <= brightness_high.get_position().y + brightness_high.get_bounds().y)
    {
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        if (util::mouse_manager::left)
            util::settings::brightness = 0.0f;
    }

    draw_text
    (
        brightness_high.get_position().x / 1.5,
        brightness_high.get_position().y / 1.5 + 20,
        brightness_high.get_text().c_str(),
        1.5f
    );

    glColor4f(0.6f, 0.8f, 1.0f, 1.0f);
    // Brightness medium button

    if (util::mouse_manager::x >= brightness_medium.get_position().x &&
        util::mouse_manager::x <= brightness_medium.get_position().x + brightness_medium.get_bounds().x &&
        util::mouse_manager::y >= brightness_medium.get_position().y &&
        util::mouse_manager::y <= brightness_medium.get_position().y + brightness_medium.get_bounds().y)
    {
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        if (util::mouse_manager::left)
            util::settings::brightness = 0.2f;
    }

    draw_text
    (
        brightness_medium.get_position().x / 1.5,
        brightness_medium.get_position().y / 1.5 + 20,
        brightness_medium.get_text().c_str(),
        1.5f
    );

    glColor4f(0.6f, 0.8f, 1.0f, 1.0f);
    // Brightness low button

    if (util::mouse_manager::x >= brightness_low.get_position().x &&
        util::mouse_manager::x <= brightness_low.get_position().x + brightness_low.get_bounds().x &&
        util::mouse_manager::y >= brightness_low.get_position().y &&
        util::mouse_manager::y <= brightness_low.get_position().y + brightness_low.get_bounds().y)
    {
        glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
        if (util::mouse_manager::left)
            util::settings::brightness = 0.4f;
    }

    draw_text
    (
        brightness_low.get_position().x / 1.5,
        brightness_low.get_position().y / 1.5 + 20,
        brightness_low.get_text().c_str(),
        1.5f
    );
